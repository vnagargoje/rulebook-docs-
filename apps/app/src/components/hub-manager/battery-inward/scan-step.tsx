...
export function ScanStep({ selectedMovement, scannedBatteries, onScan, onRemove, onProceedToConfirm }: ScanStepProps) {
    return (
        <View className='flex-1'>
            {/* Selected Movement Info */}
            <View className='mx-4 mt-4 rounded-2xl border border-primary-100 bg-primary-50 px-4 py-3'>
                <Text className='text-xs font-semibold text-primary-700'>
                    From: {selectedMovement?.fromStation?.name ?? selectedMovement?.fromStationId}
                </Text>
                <Text className='mt-0.5 text-xs text-primary-500'>
            ...
            <View className='flex-1 px-4 mt-4'>
                <View className='flex-row items-center justify-between mb-2'>
                    ...
                    <View className='rounded-full bg-primary-100 px-2.5 py-0.5'>
                        <Text className='text-xs font-bold text-primary-600'>{scannedBatteries.length}</Text>
                    </View>
                </View>
                <ScrollView
