...
import { ActivityIndicator, FocusAwareStatusBar, SafeAreaView, Text, View } from '@/components/ui'
...
export default function PlansScreen() {
    ...
    const [activeTab, setActiveTab] = useState<TabKey>(
        params.tab === 'topups' ? 'topups' : 'plans',
    )
            ...
            <SafeAreaView className='flex-1 bg-background'>
                <FlatList
                    ...
                    renderItem={({ item }) => {
                        ...
                        if (activeTab === 'plans') {
                            return <PlanCard plan={item} onPress={() => handlePlanPress(item.id)} />
                        }

                        return <TopUpCard topUp={item} onPress={() => handleTopUpPress(item.id)} />
                    }}
                    ItemSeparatorComponent={() => <View className='h-3' />}
                    showsVerticalScrollIndicator={false}
                    refreshControl={<RefreshControl refreshing={plansRefetching || topUpsRefetching} onRefresh={handleRefresh} />}
                    ...
                    ListEmptyComponent={
                        activeTab === 'plans' ? (
                            ...
                            <View className='items-center rounded-3xl border border-neutral-200 bg-white py-12'>
                                <MaterialCommunityIcons name='battery-plus-outline' size={48} color='#D1D5DB' />
                                <Text className='mt-4 text-lg font-semibold text-neutral-900'>
