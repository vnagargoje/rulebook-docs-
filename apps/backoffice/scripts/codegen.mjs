...
const apiUrl = process.env.VITE_API_URL || 'http://localhost:4500';
const output = path.resolve(__dirname, '../app/services/api/codegen');
const name = 'Api.ts';
const templates = path.resolve(__dirname, '../../../scripts/swagger-templates');

const cleanUrl = apiUrl.endsWith('/') ? apiUrl.slice(0, -1) : apiUrl;

console.log(`🚀 Generating Backoffice API client from: ${cleanUrl}/openapi.json`);
